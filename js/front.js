                    ...
                    .load(function(){
						...
						if (nbImages == i) {
							ready(_fnReady);
	                         ...
	                         percent = i/nbImages;
	                         self =  this;
	                         if(typeof _callbackFn != 'undefined' && typeof _callbackFn == 'function') _callbackFn(percent,i,self,v,w);
					});

				return $item.error(), $item.load();
...
;(function (window, undefined) {
     ...
     * Constructor.
     */
    var Momentum = function(_el,_dragInstance,_options)
    {
        var _instance               =   this;
        this.startX                 =   0;
        this.endX                   =   0;
        ...
        this.tween                  =   false;
        this.locked                 =   false;
        this.draggable              =   _dragInstance[0];
        this.options                =   _options || {};
        this.options.ease           =   this.options.ease || Power3.easeOut;
        this.options.duration       =   this.options.duration || .5;
        this.options.smoothBounds   =   this.options.smoothBounds || false;
        return _instance;
    }
    Momentum.prototype.startPosition = function(_event)
    {
        this.startX     =   _event.clientX;
    }
    Momentum.prototype.endPosition = function(_event)
    {
        this.endX     =   _event.clientX;
    }
    Momentum.prototype.isLocked = function()
    {
        var
            scope       =   this
        ;
        scope.locked    =   true;
        scope.draggable.disable();

    }
    Momentum.prototype.isEnabled = function()
    {
        var
            scope       =   this
        ;
        scope.locked    =   false;
        scope.draggable.enable();
    }
    Momentum.prototype.get  =   function()
    {
        var
            scope       =   this
        ;
        ...
        console.info('start : ',this.startX,'end :',this.endX);
    }
    Momentum.prototype.getInertia = function()
    {
        var
            scope           =   this
        ,   datas           =   scope.get()
        ,   inertia         =   datas.x.start - datas.x.end
        ;

        if(datas.x.start > datas.x.end)
        {
            scope.directionX    =   1;
        }
        else
        ...
        if(!scope.locked) scope.animateX(inertia);
    }
    Momentum.prototype.animateFlickX = function(_from)
    {
        var
            scope               =   this
        ,   $ref                =   scope.container.find('> div')
        ;
        ...
        if(_from < 0)
        {
            ...
            .fromTo(
                ...
                {
                    x : -(_from * scope.draggable.edgeResistance)
                },
                {
                    x : 0,
                    ease : scope.options.ease
                }
            );
        }

    }
    Momentum.prototype.animateX = function(_inertia)
    {
        var
            scope           =   this
        ,   getScrollLeft   =   scope.container.scrollLeft()
        ,   newScrollLeft   =   getScrollLeft + _inertia
        ,   boundRightX     =   scope.container.find('.nested').width() - scope.container.width()
        ;

        // smoothBounds
        if(scope.options.smoothBounds)
        {
            if(newScrollLeft > boundRightX)
            {
                newScrollLeft   =   boundRightX;
            }
        ...
        // Presets
        scope.isLocked();
        if(scope.draggable.edgeResistance != 0 && !scope.options.smoothBounds) scope.animateFlickX(newScrollLeft);

        // tween
        scope.tween     =   TweenLite
        .fromTo(
            scope.container,
            scope.options.duration,
            {
...
;(function (window, undefined) {
    ...
    $(function ()
    {
        ...
        {           
            ...
            // try to unload current controller
            _k.fire( _k.controller, 'unload');

            // BEFORE
            _k.publish('statechange::before');

            // Start Fade Out
            // Animating opacity to 0 still keeps the element's height intact
            // Which prevents that annoying pop bang issue when loading in new content
            $content.animate({ opacity: ns.ajax.opacity}, ns.ajax.durationFadeIn, function ()
            {
                $pageTop.scrollTop(0);
                ...
                $.ajax(
                {
                    ...
                    success: function (_json, textStatus, jqXHR)
                    {
                        ...
                        _k.fire( _json.js.controller );

                        // store current controller for unload
                        _k.controller = _json.js.controller;

                        if( typeof _json.js.method != 'undefined' && _json.js.method !== '' )
                        {
                            _k.fire(_json.js.controller, _json.js.method);
                        }
                        ...
                        _k.publish('statechange::finalize');
                    },
...
$(function () {
    ...
    if (IS_IPAD || IS_IPHONE) {
        ...
        $('.subnav li a').on('click touchend', function() {
...
PJM.core = (function()
{
    ...
    var detectBrowser = function() {
        if(navigator.userAgent.toLowerCase().indexOf('applewebkit')!=-1 && navigator.userAgent.toLowerCase().indexOf('macintosh')!=-1) {
     ...
     */
    var moveMainNav = function() {
        scrollTop = $window.scrollTop();
        if(scrollTop >= 0) {
            var navtop = scrollTop <= 41 ? (41-scrollTop)+'px' : 0;
            $mainNav.css({top: navtop});
        }
    }
    ...
    var ready = function() {
        ...
        if($('#awards').length || $('#editeurs').length) {
            PJM.modules.scrollTo();
        }
    }
...
})();;PJM.toolbox = (function()
{

    var animBG = {

        /*
        * Get the marginLeft position of the img in order to center it in the window
        * Parameters:
        *   $listImg : image element, $win : window, delta: space between default position and $listImg position
        */
        getImgPosition: function($listImg, $win, delta){
            if($listImg.width() > $win.width()){
                var diff        = $listImg.width() - $win.width();
                var diffLeft    = Math.floor(diff/2);
                var initPos     = -diffLeft+delta;
                return initPos;
            }
        },
        /*
        * Center Img with effect
        * Parameters:
        *   $listImg : image element, $win : window, delta: space between default position and $listImg position, bounce: space on animation
        */
        positionImg: function($listImg, $img, $win, delta, bounce, onload){

            var x = animBG.getImgPosition($listImg, $win, delta);
            $listImg.css({'marginLeft': x, "left": "auto" }).fadeIn();

            /* Creating an intro sequence to animate the timeline */
            if(onload == true){

                for(var i=0; i<$img.length; i++){
                    $img[i].style.left = ($img.width()*i)+'px';
                }

                var t1 = new TimelineLite({onComplete: animBG.launchMouseOver(delta), smoothChildTiming: false});

                t1.add( TweenLite.to($listImg, 1, {css: {marginLeft: x - bounce}, ease: Power0.easeIn, delay:0.1 }) );
                t1.add( TweenLite.to($listImg, 1.2, {css: {marginLeft: x }, ease: Power0.easeIn, delay:0.1  }) );

                t1.pause();
                t1.resume();

            }else{
                animBG.launchMouseOver(delta);
            }
        },
        /*
        * On mouse over of #left div, this scolls left
        */
        animateOnLeft: function($zone, $img, $listImg, $vitesse){
            var interval, tweenLeftIn, $newLeft;
            ...
            $zone.on('mouseover', function(){
                ...
                for(var i= 0; i<$img.length; i++){
                    posLeft[i] = parseInt($img[i].style.left);

                    // on anime l'image
                    $newLeft = posLeft[i]+$vitesse;
                    tweenLeftIn = new TweenLite($img[i], 1,{css: {left: $newLeft} , ease:Linear.easeNone });
                }

                interval = setInterval(function(){
                    console.log('$first '+$img[0].style.left);
                    for(var i= 0; i<$img.length; i++){
                        posLeft[i] = parseInt($img[i].style.left);
                        ...
                        // Si première élément de $img est trop sur la droite (décalage de la moitié de l'image)
                        if((i == 0) && (posLeft[i] >= $limit)){
                            // l'image la plus a droite est placé tout a gauche
                            $newLeft = parseInt($img[0].style.left) - $imgWidth;
                            tweenLeftIn = new TweenLite($img[$last], 0,{css: {left: $newLeft} , ease:Linear.easeNone });
                            // on lui assigne une nouvelle animation prenant en compte sa nouvelle position
                            $newLeft = $newLeft+$vitesse;
                            tweenLeftIn = new TweenLite($img[$last], 1,{css: {left: $newLeft} , ease:Linear.easeNone });

                            // on place l'element qui a changer de place (dernier li) en premier li
                            $listImg.find('li:last').detach().insertBefore(".visual-top li:first");
                            // reselectionne les li pour que l'index i soit bon lors de la boucle for
                            $img = $listImg.find('li');
                        }
                    }
                },1000)
            ...
            $zone.on('mouseout', function(){
                ...
                var tweenLeftOut, posLeft;
                for(var j=0; j<$img.length; j++){
                    posLeft = parseInt($img[j].style.left);
                    tweenLeftOut = new TweenLite($img[j], 0,{css: {left: posLeft} , ease:Power2.easeInOut })
                    .pause()
                    .resume();
                }
            });
        ...
        /*
        * On mouse over of #rigth div, this scolls right
        */
        animateOnRight: function($zone, $img, $listImg, $vitesse){
            var interval, tweenRightIn, $newLeft;
            var $imgWidth = $img.width();
            var $limit = 0;
            var $last = ($img.length - 1);
            var posLeft = new Array();
            ...
            $zone.on('mouseover', function(){
                ...
                for(var i= $last; i>=0; i--){
                    ...
                    // on anime l'image
                    $newLeft = posLeft[i]-$vitesse;
                    tweenRightIn = new TweenLite($img[i], 1,{css: {left: $newLeft} , ease:Linear.easeNone });
                }
                ...
                interval = setInterval(function(){
                    for(var i= $last; i>=0; i--){
                        ...
                        // Si dernière élément de $img est trop sur la gauche (décalage de la moitié de l'image)
                        if((i == 0) && (posLeft[i] <= $limit)){
                            // l'image la plus a gauche est placé tout a droite
                            $newLeft = parseInt($img[$last].style.left) + $imgWidth;
                            tweenRightIn = new TweenLite($img[0], 0,{css: {left: $newLeft} , ease:Linear.easeNone });
                            // on lui assigne une nouvelle animation prenant en compte sa nouvelle position
                            $newLeft = $newLeft-$vitesse;
                            tweenRightIn = new TweenLite($img[0], 1,{css: {left: $newLeft} , ease:Linear.easeNone });

                            // on place l'element qui a changer de place (premier li) en dernier li
                            $listImg.find('li:first').detach().insertAfter(".visual-top li:last");
                            // reselectionne les li pour que l'index i soit bon lors de la boucle for
                            $img = $listImg.find('li');
                        }
                    }
            ...
            $zone.on('mouseout', function(){
                ...
                var tweenRightOut, posLeft;
                for(var j=0; j<$img.length; j++){
                    posLeft = parseInt($img[j].style.left);
                    tweenRightOut = new TweenLite($img[j], 0,{css: {left: posLeft} , ease:Power2.easeInOut })
                    .pause()
                    .resume();
                }
            });
        },
        draggable: function($listImg){
            Draggable.create(
                $listImg,
                {
                    type:"left",
                    bounds: {left: $listImg.position().left, width: $(window).width()}
            });

        },
        launchMouseOver: function(delta){
            var
                $container = $('#image-slider')
            ,   $listImg        = $container.find('.visual-top')
            ,   $img            = $container.find('.visual-top li')
            ,   $leftZone       = $container.find('.leftZone')
            ,   $rightZone      = $container.find('.rightZone')
            ,   $vitesse = 300 // px/sec
            ;

            if( $('html').hasClass('tablet') == false){
                /* On mouseover on left or right zones on window : we scroll right or left */
                animBG.animateOnLeft($leftZone, $img, $listImg, $vitesse);
                animBG.animateOnRight($rightZone, $img, $listImg, $vitesse);
            }else{
                animBG.draggable($listImg);
            }
        },
        init: function(onload){
            ...
            TweenLite.killTweensOf($listImg);
            $container.find('img').each(function() {
                getSrc.push($(this).attr('src'))
            });

            Preloader.initialize(getSrc,function() {
                animBG.positionImg($listImg, $img, $(window), delta, bounce, onload);
            });
        }
    };
    ...
    var Filter = function(){
        ...
        ,   $ico        = $button.find('.ico')
        ,   $inputList  = $form.find('input[type=checkbox]')
        ,   active      = false
        ;
            ...
            $.ajax({
                ...
                success: function(jsonData) {
                    ...
                    setTimeout(function() {
                        ...
                        // $('#teamload').fadeOut().delay(500);

                        // LAUNCH SLIDESHOW FOR PRESSE
                        if($('#slideshow-full')) {
                            PJM.slideshow.init();
                        }
                    }, 500);
    ...
    // Animate top nav elements
    var MainSearch = function(){
        var
            $searchButton       = $('#search')
        ,   $searchForm         = $('#main-search')
        ,   $newsletter         = $('li.newsletter')
        ,   $newsletterButton   = $newsletter.find('a')
        ,   $newsletterForm     = $('#newsletter')
        ,   activeState         = false
        ,   $newsletterButtonFoo= $('li.newsletter-footer a')
        ,   $newsletterFormFoo  = $('#newsletter-footer')
        ;
        ...
        $searchForm.submit(function(e) {
            ...
            $keyword.css({'border-color':'#E8E7E7'});
            if(!$keyword.val()) {
        ...
        $searchButton.click(function(e){
            e.preventDefault();
            var state = checkState($searchForm);

            // SHOW SEARCH FORM & HIDE NEWSLETTER FORM (IF OPEN)
            if(state == false){
                if(checkState($newsletterForm)) {
                    $newsletterForm.css('z-index', '-1');
                    $newsletterButton.animate({'margin-right': '0'}, function() {
                        $newsletterForm.fadeOut(0.1);
                    });
                }
                $searchForm.fadeIn(0.1, function() {
                    $searchButton.animate({'margin-right': '190px'}, function() {
            ...
            // HIDE SEARCH FORM
                ...
                $searchForm.css('z-index', '-1');
                $searchButton.animate({'margin-right': '0'}, function() {
                    $searchForm.fadeOut(0.1);
                });
            }
        });
        ...
        $newsletterButton.click(function(e) {
            e.preventDefault();
            var state = checkState($newsletterForm);

            // SHOW NEWSLETTER FORM & HIDE SEARCH FORM (IF OPEN)
            if(state == false){
                if(checkState($searchForm)) {
                    $searchForm.css('z-index', '-1');
                    $searchButton.animate({'margin-right': '0'}, function() {
                        $searchForm.fadeOut(0.1);
                    });
                }
                $newsletterForm.fadeIn(0.1, function() {
                    $newsletterButton.animate({'margin-right': '190px'}, function() {
                        $newsletterForm.css('z-index', '1');
                    });
                });
                //$newsletterButton.fadeOut(0.1);
            }
            // HIDE NEWSLETTER FORM
                ...
                //$newsletterButton.fadeIn(0.1);
            }
        });
        $newsletterButtonFoo.click(function(e) {
            e.preventDefault();
            var state = checkState($newsletterFormFoo);
            if(state == false){
                $newsletterFormFoo.fadeIn(0.1, function() {
                    $newsletterFormFoo.animate({'left': '100px'});
                });

            }else{
                $newsletterFormFoo.animate({'left': '-100px'}, function() {
                    $newsletterFormFoo.fadeOut(0.1);
                });
            }
        });
        $newsletterForm.submit(function(e) {
            ...
            if(url) {
                $.ajax({
                    ...
                    beforeSend: function() {
                        $newsletterForm.find('input[name=email]').css({'color':'#000', 'border-color':'#E8E7E7'});
                    },
                    success: function(jsonData) {
                        // PARSE JSON
                        var data = false;
                        ...
                        setTimeout(function() {
                            if(data) {
                                if(data[0].level == 'error') {
                                    $newsletterForm.find('input[name=email]').css({'color':'red', 'border-color':'red'});
                                }
                                if(data[0].level == 'success') {
                                    $newsletterForm.find('input[name=email]').css({'color':'green', 'border-color':'green'});
                                    setTimeout(function() {
                                        $newsletterForm.fadeOut(0.1);
                                        $newsletterButton.fadeIn(0.1);
        ...
        $newsletterFormFoo.submit(function(e) {
            e.preventDefault();

            var url = $newsletterFormFoo.attr('action');
            if(url) {
                $.ajax({
                    url: url,
                    type: 'post',
                    data: $newsletterFormFoo.serialize(),
                    beforeSend: function() {
                        $newsletterFormFoo.find('input[name=email]').css({'color':'#000', 'border-color':'#E8E7E7'});
                    },
                    success: function(jsonData) {
                        // PARSE JSON
                        var data = false;
                        ...
                        setTimeout(function() {
                            if(data) {
                                if(data[0].level == 'error') {
                                    $newsletterFormFoo.find('input[name=email]').css({'color':'red', 'border-color':'red'});
                                }
                                if(data[0].level == 'success') {
                                    $newsletterFormFoo.find('input[name=email]').css({'color':'green', 'border-color':'green'});
                                    setTimeout(function() {
                                        $newsletterFormFoo.fadeOut(0.1);
                                        $newsletterFormFoo[0].reset();
                                        $newsletterFormFoo.find('input[name=email]').css({'color':'#000', 'border-color':'#E8E7E7'});
                                    }, 2000);
                                }
                            }
    ...
    // Set Height for home page elements
    var displayMainHomeElements = function(){
        var
            $ref        = $('.homeSelection')
        ,   $item       = $ref.find('article')
        ...
        $ref.height((itemLength/2)*itemHeight + (itemLength/2)*gutter);
    };

    // Custom select for selectBox
    var selectBox = function(){
        $('#contact-form select').selectBox();
    };

    var hoverMainNav = function(){

        $('#main-nav li').not(":eq(0)").each(function(){
            $(this).mouseenter(function(){
                $('.overlay-subnav').css({ height: '39px'});
            });
        ...
        $('#main-nav .menu').mouseleave(function(){
            $('.overlay-subnav').css({ height: '0'});
        });

        $('#main-nav li').eq(0).hover(function(){
            $('.overlay-subnav').css({ height: '0'});
        });
    };

    //Animation for elements at homepage
    var animVisualHomePage = function(){
        var
            $scope      = $('.homeSelection')
        ,   $left       = $scope.find(".half.left")
        ,   $right      = $scope.find(".half.right")
        ,   duration    = 1
        ;
        ...
        $left.each(function(){
            ...
            $(this).hover(function(){
                ...
                var
                    $thisRight  = $this.next('.right')
                ;

                //console.log($thisRight);

                TweenLite.to($this, duration, {width: '80%', ease:Power2.easeOut, onStart: function(){
                    $this.css('z-index', 9999);
                    $this.addClass('active');
                    $thisRight.removeClass('active');
                    $thisRight.css('z-index', 1);
                }
                });

            }, function(){

                TweenLite.to($this, duration, {width: '50%', ease:Power2.easeOut, onComplete: function(){
        ...
        $right.each(function(){
            ...
            $this.hover(function(){
                var
                    $thisLeft  = $this.prev('.left')
                ;

                TweenLite.to($this, duration, {width: '80%', ease:Power2.easeOut, onStart: function(){
                    $this.css('z-index', 9999);
                    $this.addClass('active');
                    $thisLeft.removeClass('active');
                    $thisLeft.css('z-index', 1);
                }});
            }, function(){
...
;(function(window, document, undefined) {
    ...
    PJM.timeline = (function()
    {
        ...
        var Module = function()
        {
            ...
            this.view           =   false;
            this.gallery        =   false;
            this.anchorsNav     =   false;
            this.wheelEvent     =   'mousewheel.timeline';
            this.velocity       =   1.5;
            this.steps          =   [];
            this.startAccess    =   0;
            this.init();
        ...
        Module.prototype =
        {
            actions : function()
            {
                ...
                var
                    scope       =   this
                ;
                scope.anchorsNav.find('li').on('click',function() {
                    var
                        $ref    =   $(this)
                    ,   dataPos =   $ref.data('posx')
                    ;
                    scope.moveTo(dataPos);
                });
                $('.btn-timeline').on('click',function() {
                    scope.view.addClass('active')
                });
                $('.btn-close').on('click',function() {
                    scope.view.removeClass('active')
                });
                scope.view.find('.btn-previous').on('click',function() {
                   
                    var
                        $getPrevious    =   scope.anchorsNav.find('li.active').prev()
                    ,   dataPos         =   $getPrevious.data('posx')
                    ;
                    scope.moveTo(dataPos);
                });
                scope.view.find('.btn-next').on('click',function() {
                    var
                        $getPrevious    =   scope.anchorsNav.find('li.active').next()
                    ,   dataPos         =   $getPrevious.data('posx')
                    ;
                    if(dataPos) {
                        scope.moveTo(dataPos);
            ...
            getAnchor : function()
            {
                var
                    scope           =   this
                ,   getScrollLeft   =   scope.gallery.scrollLeft()
                ;
                $.each(scope.steps,function(i,el) {
                    if(getScrollLeft<=el)
                    {
                        scope.updateAnchor(i);
                        return false;
                    }
                });
            }
            ...
            initPosition : function()
            {
                var
                    scope           =   this
                ,   getPosX         =   scope.anchorsNav.find('li').eq(scope.startAccess).data('posx')
                ;

                scope.moveTo(getPosX);

                var
                    startGallery        = scope.gallery.find('li').eq(scope.startAccess).offset().left
                ,   startGalleryWidth   = scope.gallery.find('li').eq(scope.startAccess).width()
                ,   startX              = Math.floor(startGallery + (startGalleryWidth/2))
                ;
                //console.log("scope "+scope.width());
               // scope.moveTo(startX);
            }
            ,
            make : function() {
           ...
           preload : function()
            {
                ...
                scope.gallery.find('img').each(function() {
                    getSrc.push($(this).attr('src'));
                    
                    var imgWidth = $(this).width();
                    widthList += imgWidth+1;
                    $(this).parents('li').css('width', imgWidth);
                });

                scope.gallery.find('ul').css('width', widthList);
            ...
            resize : function()
            {
                ...
                var
                    scope   =   this
                ,   $last   =   scope.gallery.find('li:last-child')
                ,   total   =   Math.round($last.position().left + $last.width())
                ;
                scope.gallery.children()
                    .css({
                        'width':total,
                ...
                draggable   = Draggable.create(
                    $ref,
                    {
                        ...
                        onDragStart : function(_e) {
                            momentum.startPosition(_e);
                        }, onDragEnd : function(_e) {
                            momentum.endPosition(_e);
                            momentum.getInertia();
                        }
                });
                momentum =  new Momentum($ref,draggable,{duration:.8,smoothBounds:true});
            }
            ,
            setup : function()
            {
                this.view           =   $('#timelineView');
                this.gallery        =   this.view.find('.gallery');
                this.anchorsNav     =   this.view.find('.anchors');
                this.constructor    =   true;

                // FIND ACTIVE ITEM (LEGEND CONTAIN %$%)
                var starter = this.gallery.find('.started').index();
                if(starter != -1) {
                    this.startAccess = starter;
                }
            }
            ,
            step : function()
            {
                var
                    scope       =   this
                ,   diff        =   scope.gallery.find('.nested').width() - scope.gallery.width()
                ,   ratio       =   scope.gallery.find('.nested').width() / diff
                ,   ratio       =   Math.round(ratio*100)/100
                ,   totalLi     =   this.gallery.find('li').length - 1
                ;
                this.gallery.find('li').each(function(i) {
                    var
                        posX    =   Math.round($(this).position().left / ratio)
                    ;
                    scope.steps.push(posX)
                    if(i == totalLi)
                    {
                        scope.anchorsNav.find('li').eq(i).data('posx',diff);
                    }
                    else
                    {
                        scope.anchorsNav.find('li').eq(i).data('posx',posX);
                    }
                });
            }
            ,
            updateAnchor : function(_index)
            {
                this.anchorsNav.find('li').removeClass('active');
                this.anchorsNav.find('li').eq(_index).addClass('active');
            }
            ,
            wheelOn : function()
            {
                ...
                ,   $ref    =   scope.gallery
                ;
                $ref.on(scope.wheelEvent, function(_e) {
                    var
                        Event   =   _e
                    ;
                    this.scrollLeft += (Event.deltaX * scope.velocity);
                    // Event.preventDefault();
                });
            }
            ,
            displayMiniature : function()
            {
                ...
                var
                    scope                = this.anchorsNav
                ,   $anchor              = scope.find('li')
                ,   $miniature           = $anchor.find('.miniature')
                ,   $triangle            = $miniature.find('.triangleUp')
                ,   anchorWidth          = 20
                ;

                $triangle.each(function(){
                    var imgMiniatureWidth = $(this).parent('.miniature').find('img').width();
                    $(this).css({
                        'left' : ( Math.round(imgMiniatureWidth) / 2 ) - 2
                    ,   'top'  : -4
                    });
                });

                $miniature.each(function(){
                    var miniatureWidth = $(this).find('img').width();
                    $(this).css({
                        'width' : miniatureWidth
                    ,   'left'  :  - ( miniatureWidth / 2) + (anchorWidth / 2)
                    });
                });

                $anchor.each(function(){
                    $(this).hover(function(){
                        TweenLite.to($(this).find('.miniature'), 1, { autoAlpha:1, delay:0.5 });
                    }, function(){
                        TweenLite.to($(this).find('.miniature'), 1, { autoAlpha:0, delay:0.5 });
                    });
                });
            ...
            displayOverlay : function(){
                var
                    scope       = this.view
                ,   $features   = scope.find('.features')
                ,   $parents    = $features.parent('li')
                ;
                ...
                $parents.each(function(){
                    $(this).hover(function(){
                        $(this).find('.features').css('bottom', 0);
                    }, function(){
                        $(this).find('.features').css('bottom', -($(this).height()));
                    });
                });
            },
            init : function()
        ...
        function getPosition() {
            rafListener = requestAnimationFrame(getPosition);
            timelineModule.getAnchor();
        }

        // setTimeout(function() {
        //     cancelAnimationFrame(rafListener);
        // }, 3000);
    ...
    PJM.slideshow = (function()
    {
        ...
        $.Slider.prototype = {
            _init: function(options) {
                this._launch(options);
            },
            _show: function()
            {
                slideshowConteneur.fadeIn(500);
            },
            ...
            _launch: function(options)
            {
                var self = this;

                slideshowConteneur = $('#slideshow-full');
                slideshowLinkLists = $('.presse_link');


                slideshowLinkLists.click(function(event) {
                    event.preventDefault();

                    //on recupere la gallerie correspondante a l'image cliqué
                    $.ajax({
                        type: "POST",
                        url: window.location.href,
                        data: {todo: 'slideshow', href: $(this).attr('href')}
                    })
                            .success(function(json) {
                                //recupere le json
                                var presseJSon = jQuery.parseJSON(json);
                                //on insere le code HTML dans le slider
                                slideshowConteneur.html(presseJSon[0].msg);
                                // options
                                self.options = $.extend(true, {}, $.Slider.defaults, options);
                                // cache some elements and initialize some variables
                                self._config();
                                // initialize/bind the events
                                self._initEvents();
                                // cache le filtre si il est actif
                                self._hideFilter();
                                //on affiche le slider
                                $.Slider.prototype._show();
                            });
                });
            ...
            _config: function() {
                ...
                this.$btnclose = $('<div class="btn-close"></div>'),
                        this.$conteneurImage = $('.conteneurImage');
                this.$btnclose.appendTo(this.$conteneurImage);

                if (this.itemsCount > 1) {

                    // add navigation arrows (the previous arrow is not shown initially):
                    this.$navPrev = $('<span class="sliderprev"></span>').hide();
                    this.$navNext = $('<span class="slidernext"></span>');
                    this.$conteneurImage = $('.conteneurImage');

                    this.$navNext.appendTo(this.$conteneurImage);
                    this.$navPrev.appendTo(this.$conteneurImage);
                    // add navigation dots
                    var dots = '';
                    for (var i = 0; i < this.itemsCount; ++i) {
                        // current dot will have the class slidercurrent
                        var dot = i === this.current ? '<span class="slidercurrent" data-index="' + i + '"></span>' : '<span data-index="' + i + '"></span>';
                        dots += dot;
                    }
                    var navDots = $('<div class="sliderdots"/>').append(dots).appendTo(this.$el);
                    this.$navDots = navDots.children('span');
                    //miniatures hover
                    $(this.$navDots).mouseover(function() {
                        /*mrgnleft : calcul de la position de chaque image */
                        var mrgnleft = -22 + this.dataset.index * 22 - 22 * slideshow_preview.length;
                        /*mrgnlefttriangle : calcul de la position du triangle au dessus de l'image */
                        var mrgnlefttriangle = mrgnleft + 29;
                        // on affiche le triangle et l'image
                        $('<div class="image_preview triangle" style="margin-left:' + mrgnlefttriangle + 'px "></div>')
                                .appendTo('.sliderdots').fadeIn(500);
                        //tableau d'image créé dans mod.slideshow-full.inc.php
                        //this.dataset.index = index de chaque dots pout afficher l'image correspondante
                        $('<img src="' + slideshow_preview[this.dataset.index] + '" class="image_preview" alt="preview" style="margin-left:' + mrgnleft + 'px; "/>')
                                .appendTo('.sliderdots').fadeIn(500);
                    });
                    //lorsque l'on enleve la souris on cache la miniature et le triangle puis on les supprimes
                    $(this.$navDots).mouseout(function() {
                        $('.image_preview').fadeOut(400).queue(function() {
                            $(this).remove();
                        });
                    });
                }
            ...
            _initEvents: function() {
                ...
                slideshowClose = $('.btn-close');
                /* on ferme le slider */
                slideshowClose.click(function() {
                    slideshowConteneur.fadeOut(800);
                });
                var self = this;
                if (this.itemsCount > 1) {
                    /* click sur fleche next et prev + dots*/
                    $('.sliderprev').on('click.Slider', $.proxy(this._navigate, this, 'previous'));
                    $('.slidernext').on('click.Slider', $.proxy(this._navigate, this, 'next'));
            ...
            _toggleNavControls: function() {
                ...
                this.$navDots.eq(this.old).removeClass('slidercurrent').end().eq(this.current).addClass('slidercurrent');
            },
...
;(function(window, document, undefined) {
    ...
    PJM.modules = (function() {
        var Accordion = function() {
            if($('.module-info').length) {
                $('.module-info').each(function() {
                    $(this).find('.item').on('click',function() {
                        var
                            $ref        =   $(this)
                        ,   status      =   ($ref.hasClass('close'))?'close':'open'
                        ;
                        if(status == 'open') {
                            $ref
                                .removeClass('open')
        ...
        var scrollTo = function() {
            $('a[href^="#"]').click(function(e) {
                ...
                var dest = $('#'+id);
                if(dest.length) {
                    $('html,body').animate({scrollTop: dest.offset().top-90});
                }
            });
        }
    ...
    PJM.gen_fiche = (function() {

        // Initialize the template
        //=======================
        var init = function() {
            if($('#timelineView').length) PJM.timeline.init();
            PJM.modules.init();
        }
...
}(window, document));;// Adapt or ie8
if( $('.ie8').length > 0 || $('.ie7').length > 0 ){
	// Display placeholder for contact form
	$('input, textarea').placeholder();


	//Display overlay on projects
	if($('.layout-projects').length > 0){
		if($('.item-full').length > 0){
			$('.item-full.alt-light').prepend('<div class="overlay alt-light"></div>');
			$('.item-full.alt-dark').prepend('<div class="overlay alt-dark"></div>');
		}
	}
