...
if($_POST['action']=='save') {
	...
	if($result) {
		...
		js_a_l('儲存成功', $main_str.'_handle.php?id=' . $id . '&prev=' .$_POST['category'] . '&category='.$_POST['category2']);exit;
	}else {
...
if($category!=0) {		
	...
	$query = "select id, name, prev, lv from health_products_category2 where id='".$tmp_prev."'";
	$pc = $obj_cate1->run_mysql_out($query);
	$page_subtitle = "<a href=products2.php?prev=".$pc['prev']."&category=" . $category . "><li class='left'>".$pc['name']."</li></a>".$page_subtitle;

	$query = "select id, name, prev, lv from health_products_category where id='".$prev."'";
	$pc = $obj_cate1->run_mysql_out($query);
	$page_subtitle = "<a href=products_category2.php?prev=".$prev."><li class='left'>".$pc['name']."</li></a>". $page_subtitle;
   ...
   <div class="mainContent">
   	 <div id="data_content">
   		<form method="post" id="<?=$main_str?>_form" action="<?=$_SERVER['PHP_SELF']?>">
            ...
            <input type="hidden" name="category" value="<?=$prev?>" />                                    
            <input type="hidden" name="category2" value="<?=$category?>" />                        
            <table class="<?=$main_str?>-table" border="0">
                ...
                <tr>
                    <td width="120" height="60">商品名稱：</td>
                    <td align="left"><input type="text" name="name" id="name" value="<?=$prod['name']?>" class="frome" style="width:500px" /></td>
                </tr>                            
                <tr>
                    <td height="60">排　　序：</td>
                    <td align="left"><input type="text" name="sort" id="sort" value="<?=($prod['sort'])?$prod['sort']:0?>" class="frome" style="width:250px" /> (數字越大排序越前面)</td>
                ...
                <tr>
                    ...
                    <td style="text-align: center;">
                        ...
                        <button type="button" id="ppppp" class="editbtn enter" title="預覽" onClick="preview();">預 覽</button>
                        <?	}	?>
					</td>
                </tr>
            </table>
