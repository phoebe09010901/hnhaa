...
//subtitle
$page_subtitle = '<li class="left"><a href="album.php?category=' . $category . '">' . $title . '</a></li>';
//$page_subtitle = "<a href=album_category.php><li class='left'>" . $album_categoryRs['name'] . "</li></a>" . $page_subtitle;

$obj_keyword= new mysql_page();		
$query = "select keywords from hnhaa_system_set";	
$kw = $obj_keyword->run_mysql_out($query);//print_r($obj_photo->result_data);
		...
		success: function(request) {
			$.each(request, function(key, photo_data) {
				$.each(photo_data, function(key, data)	{
					...
					if(data['alt']=='<?=addslashes($keywords)?>')
