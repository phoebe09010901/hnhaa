...
class files {	
	...
	public function uploaded_file($tmp_name, $uppath){
		...
		if (!file_exists(dirname($uppath))) {
			mkdir(dirname($uppath), 0777);	
		} 
	...
	public function uploadify_row($type, $show_type, $i, $rowName, $dataPath, $dataFile, $dataTitle, $dataWidth_s, $dataHeight_s, $dataWidth, $dataHeight, $dataFilesize) {
		...
		if($type=='images' || $type=='files') {
			...
			if($type=='images'){  
				...
				echo '<input type="text" name="file'.$i.'" id="file'.$i.'" value="'.$dataFile.'" class="frome" style="width:200px" readonly="readonly" /><br /><input type="file" name="file'.$i.'_upload" id="file'.$i.'_upload" />(允許檔案格式: '.$allow_files_str.'，檔案大小:'.$dataFilesize.'mb)';
			}
			echo '</div>';
		}elseif($type=='photos') {
			$allow_files_str = '';
			...
			echo '<div class="upload_photo"><input type="file" name="file'.$i.'_upload" id="file'.$i.'_upload" />(選擇照片上傳，允許檔案格式: '.$allow_files_str.'，建議圖片尺寸: '.$dataWidth.'*'.$dataHeight.'，檔案大小:'.$dataFilesize.'mb)</div>';	
		}
	}
