...
$(function(){
	...
	$('#b_<?php echo $i?>').hover(function(){
		$('#p_<?php echo $i?>').stop().animate({ //滑鼠進來
			opacity: 1,
			top: '0px',
		},0);
	}, function(){
		$('#p_<?php echo $i?>').stop().animate({ //滑鼠出去
			opacity: 0,
			top:'-9999px'
		},0);
	});
...
<?
		...
		if($tmpC > 0){
			...
			while($row1 = mysql_fetch_array($rl1, MYSQL_ASSOC)){
				...
				if($j == 1){
?>
                	<div class="proPic_first" id="b_<?=$i?>">
<?					
				}	else	{
?>
                	<div class="proPic" id="b_<?=$i?>">
<?								
				}
...
?>                
                    ...
                    <div class="proPic_title color_03 font-family_01" id="p_<?=$i?>"><?=$row1['title2']?></div>
<?										
					}	else	{
?>                
                    <div class="proPic_block"><img src="/album/<?=$row1['file1']?>"></div>    
                    <a href="pro_page.php?id=<?=$row1['id']?>"><div class="proPic_title color_03 font-family_01" id="p_<?=$i?>"><?=$row1['title2']?></div></a>
<?				
					}
				}	else	{	?>                    
                    <div class="proPic_block"></div>
                    <div class="proPic_title color_03 font-family_01" id="p_<?=$i?>"><?=$row1['title2']?></div>
<?				}	?>
                </div>
<?				
